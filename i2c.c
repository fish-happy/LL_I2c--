...
//发送数据,并检测,超时则重启I2C,并返回错误
__STATIC_INLINE ErrorStatus my_I2C2_TransmitData_Check (I2C_TypeDef * I2Cx , uint8_t * p_data , uint32_t len)
{
...
ErrorStatus my_I2C_TransmitData (I2C_TypeDef * I2Cx , uint8_t * p_buf , uint32_t len , uint8_t slave_addr)
{
  ...
  status |= my_I2C2_TransmitData_Check (I2Cx , p_buf , len);
...
void my_OLED091_Init (void)
{
  ...
  my_I2C_TransmitData (I2C2 , OLED_Init_ARR , sizeof (OLED_Init_ARR) , 0x78);

  // //屏幕测试
  //512个字节全部写入0xff
  uint8_t OLED_test_ARR[513];
  OLED_test_ARR[0] = 0x40;
  memset (OLED_test_ARR + 1 , 0xff , sizeof (OLED_test_ARR) - 1);
  my_I2C_DMA_TransmitData (I2C2 , DMA1 , LL_DMA_CHANNEL_4 , OLED_test_ARR , sizeof (OLED_test_ARR) , 0x78);
  LL_mDelay (2000);
  // //512个字节全部写入0
  // memset (OLED_test_ARR + 1 , 0x00 , sizeof (OLED_test_ARR) - 1);
  // my_I2C_DMA_TransmitData (I2C2 , DMA1 , LL_DMA_CHANNEL_4 , OLED_test_ARR , sizeof (OLED_test_ARR) , 0x78);

  // uint8_t OLED_test_ARR2;
  // my_I2C_ReceiveData (I2C2 , &OLED_test_ARR2 , 1 , 0x78 , 0x40);
